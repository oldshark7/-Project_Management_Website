<x-app-layout>
    <x-slot name="header">
        <x-header-component :title="'Ubah Project Scope: ' . $project->title" icon="fa-solid fa-sitemap text-blue-600 text-lg" />
    </x-slot>

    <div class="px-4 py-2">
        <!-- Back Link -->
        <div class="mb-4">
            <a href="{{ route('projects.scope.show', $project->id) }}" class="inline-flex items-center text-xs font-semibold text-slate-500 hover:text-slate-800 transition gap-1.5">
        ...
        <form action="{{ route('projects.scope.update', $project->id) }}" method="POST" id="scopeForm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <textarea name="in_scope" id="in_scope" rows="5" 
                                          ...
                                          placeholder="Contoh:&#10;+ Modul Keuangan & Akuntansi&#10;+ Manajemen Inventaris & Gudang">{{ old('in_scope', $scope->in_scope) }}</textarea>
                                @error('in_scope')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="out_of_scope" id="out_of_scope" rows="5" 
                                          ...
                                          placeholder="Contoh:&#10;- Pemeliharaan Perangkat Keras&#10;- Pelatihan User di Luar Jakarta">{{ old('out_of_scope', $scope->out_of_scope) }}</textarea>
                                @error('out_of_scope')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <textarea name="deliverables" id="deliverables" rows="4" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Contoh:&#10;1. Dokumen Arsitektur Sistem&#10;2. Source Code Core Module">{{ old('deliverables', $scope->deliverables) }}</textarea>
                                @error('deliverables')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="acceptance_criteria" id="acceptance_criteria" rows="4" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Contoh:&#10;1. Lengkap dengan diagram ERD, disetujui CTO&#10;2. Lulus unit testing 90%">{{ old('acceptance_criteria', $scope->acceptance_criteria) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <textarea name="main_requirements" id="main_requirements" rows="3" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Contoh:&#10;- Server AWS aktif&#10;- Lisensi ERP dibayar">{{ old('main_requirements', $scope->main_requirements) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="assumptions" id="assumptions" rows="3" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Contoh:&#10;- Data legacy bersih&#10;- Stakeholder tersedia">{{ old('assumptions', $scope->assumptions) }}</textarea>
                                @error('assumptions')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="constraints" id="constraints" rows="3" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Contoh:&#10;- Budget max Rp 2M&#10;- Durasi 6 bulan">{{ old('constraints', $scope->constraints) }}</textarea>
                                @error('constraints')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="3" 
                                          class="w-full px-4 py-3 bg-slate-50/50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl text-xs text-slate-800 transition placeholder-slate-400" 
                                          placeholder="Catatan tambahan penting lainnya terkait cakupan proyek...">{{ old('notes', $scope->notes) }}</textarea>
                                @error('notes')
                                    <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p>
                                @enderror
                            </div>
            ...
            <div class="fixed bottom-0 left-0 right-0 md:left-60 bg-white/85 backdrop-blur-md border-t border-slate-100 p-4 flex flex-col sm:flex-row items-center justify-between gap-4 z-30 shadow-lg px-6 transition-all duration-300">
                ...
                <div class="flex flex-wrap items-center justify-end gap-2.5 w-full sm:w-auto">
                    <a href="{{ route('projects.scope.show', $project->id) }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-800 rounded-xl text-xs font-bold transition">
                        {{ __('Batal') }}
                    </a>
